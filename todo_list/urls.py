...
from todo_list.views import (TaskListView, do_task, TaskCreateView,
                             TaskUpdateView, TaskDeleteView, TagListView,
...
urlpatterns = [
    ...
    path(
        "tasks/update/<int:pk>/", TaskUpdateView.as_view(), name="task-update"
    ),
    path(
        "tasks/delete/<int:pk>/", TaskDeleteView.as_view(), name="task-delete"
    ),
    path(
